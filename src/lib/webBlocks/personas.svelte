<script>
    ...
    let sortedPersonas = personas.sort((a, b) => a.nombre.localeCompare(b.nombre, "es", { ignorePunctuation: true }))
</script>
...
<div style="display: flex; flex-direction: column; gap: 1rem;margin-bottom: 50px">
    ...
    <div style="display: grid;grid-template-columns: repeat(auto-fill, 300px);column-gap: 10px;row-gap: 20px">
        {#each sortedPersonas as persona}
            <div class="persona">— {persona.nombre}
	            {#if persona.bsky}
	                <a href='https://bsky.app/profile/{persona.bsky}' target="_blank">
	            ...
	            {/if}
                {#if persona.tw}
                    <a href='https://x.com/{persona.tw}' target="_blank">
                ...
                {#if persona.web}
                    <a href={persona.web} target="_blank">
